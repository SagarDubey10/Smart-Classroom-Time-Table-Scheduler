...
def manage():
    ...
    if request.method == 'POST':
        # This part of the code handles form submissions for the schedule configuration
        slots_data = []
        ...
        for i in range(len(start_times)):
            ...
            # The break_name list might have empty strings for lecture slots, so we handle that
    ...
    # This part of the code handles the initial GET request for the manage page
...
@app.route('/api/admin/schedule_config_submit', methods=['POST'])
def api_update_schedule_config():
    db = get_db()
    cur = db.cursor()
    
    # Get all form data from the schedule configuration section
    start_times = request.form.getlist('start_time')
    end_times = request.form.getlist('end_time')
    is_breaks = request.form.getlist('is_break')
    break_names = request.form.getlist('break_name')
...
def download_pdf(class_name):
    ...
    for slot in SLOTS:
        if slot['is_break']:
            row_data = [f"{slot['start_time']} - {slot['end_time']}", slot['break_name']]
        else:
    ...
    table_width = landscape(A4)[0] - 2 * 72
