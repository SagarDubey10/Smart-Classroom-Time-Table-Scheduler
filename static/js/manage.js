document.addEventListener('DOMContentLoaded', () => {
  ...
  function addSlotRow(slot = {}) {
    ...
    newRow.innerHTML = `
            ...
            <select class="form-control" name="is_break">
                ...
                <option value="2" ${slot.is_break === 2 ? 'selected' : ''}>Reserve</option>
            </select>
            <input type="text" class="form-control break-name-input" name="break_name" placeholder="Break Name (optional)" value="${slot.break_name || ''}" ${slot.is_break !== 1 ? 'disabled' : ''}>
  ...
  // This part was mistakenly removed and is now restored.
  // It handles the timetable display and modal on the manage page.
  const editSlotModalElement = document.getElementById('editSlotModal');
  if (editSlotModalElement) {
    const editSlotModal = new bootstrap.Modal(editSlotModalElement);
